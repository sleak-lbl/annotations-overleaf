...
There is no technical issue preventing someone from cataloging a dataset 
by manually writing and publishing RDF files, the process is laborious 
and error prone: a good candidate for automation. Similarly, queries 
can be run on the graph through any SPARQL endpoint, but the learning 
curve is significant.

To enable easy use of the RDF graph we have developed a framework, in 
...
basic query abilities. The package is extensible: handlers for 
file and log format types follow a small interface
...
and the subjects for a tree of log data can be inferred from 
